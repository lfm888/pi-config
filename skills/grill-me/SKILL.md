...
For each question, provide 2–4 concrete options representing the most likely answers or directions. Format them clearly (e.g., A / B / C / D or numbered list) so the user can quickly pick one or type a custom answer. Think about what the user would realistically choose — generic options like "Yes" / "No" aren't helpful unless the question is genuinely binary. The user can always respond with something custom instead of picking a listed option.
